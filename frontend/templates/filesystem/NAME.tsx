...
export class NAME extends Filesystem {
	name: string;
	cacheVersion: number = 0;
	componentVersion: number = -1;
	cachedComponent: React.ReactElement = (<div key="NAME" />);
	...
	async mkdir(path: string): Promise<boolean> {
		throw new NotImplementedError("NAME doesn't support mkdir");
	}

	async writeFile(path: string, contents: ArrayBuffer): Promise<boolean> {
		throw new NotImplementedError("NAME doesn't support writes");
	}

	async rm(path: string): Promise<boolean> {
		throw new NotImplementedError("NAME doesn't support rm");
	}

	async rmdir(path: string): Promise<boolean> {
		throw new NotImplementedError("NAME doesn't support rmdir");
	}
}
