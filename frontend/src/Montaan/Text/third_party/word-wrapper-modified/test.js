...
test('wraps monospace glyphs by columns', function(t) {
	...
	t.equal(
		wrap(nowrap, { mode: 'nowrap' }),
		'this text  \nonly wraps \nnewlines',
		'eats starting whitespace'
	);

	t.equal(wrap(''), '');
	t.equal(wrap('this is not visible', { width: 0 }), '', 'zero width results in empty string');
	t.equal(
		wrap('this is not visible', { width: 0, mode: 'pre' }),
		'',
		'zero width results in empty string'
	);
	t.equal(
		wrap('this is not\nvisible', { width: 0, mode: 'nowrap' }),
		'this is not\nvisible',
		'zero width nowrap does not result in empty string'
	);
...
test('custom compute function', function(t) {
	...
	var text = 'some lines';
	t.equal(
		wrap(text, { width: 20, measure: compute2 }),
		'some\nline\ns',
		'cuts text with variable glyph width'
	);
	t.end();
});
...
test('wraps text to a list of lines', function(t) {
	...
	t.deepEqual(
		lines('the quick brown', { width: 10 }),
		expected,
		'returns a list of substring indices'
	);
	t.end();
});
