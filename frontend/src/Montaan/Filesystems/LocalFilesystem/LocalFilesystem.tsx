// src/Montaan/Filesystems/LocalFilesystem/LocalFilesystem.tsx
...
import styles from './LocalFilesystem.module.scss';

export default class LocalFilesystem extends Filesystem {
	name: string;
	cacheVersion: number = 0;
	componentVersion: number = -1;
	cachedComponent: React.ReactElement = (<div key="LocalFilesystem" />);
	...
	async mkdir(path: string): Promise<boolean> {
		throw new NotImplementedError("LocalFilesystem doesn't support mkdir");
	}

	async writeFile(path: string, contents: ArrayBuffer): Promise<boolean> {
		throw new NotImplementedError("LocalFilesystem doesn't support writes");
	}

	async rm(path: string): Promise<boolean> {
		throw new NotImplementedError("LocalFilesystem doesn't support rm");
	}

	async rmdir(path: string): Promise<boolean> {
		throw new NotImplementedError("LocalFilesystem doesn't support rmdir");
	}
}
