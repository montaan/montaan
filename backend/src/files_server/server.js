...
function handleProcessRequest(request, response) {
	...
	exec("ps Ao ppid,pid,%cpu,%mem,comm", function(error, stdout, stderr) {
		...
		procs.forEach((procInfo) => {
			const ppid = procInfo[0],
				pid = procInfo[1],
				cpu = procInfo[2],
				mem = procInfo[3],
				name = procInfo[4];
			const parent = procTree[ppid];
			const proc = { parent: parent, pid: pid, cpu: cpu, mem: mem, name: name, children: [], files: [] };
			if (parent) {
		...
		exec("lsof -Fpn0 /", function(error, stdout, stderr) {
			const procFiles = stdout.split("\0");
			let pid = 0;
			procFiles.forEach(pf => {
				...
				if (pf.charAt(0) === 'p') {
					...
					const proc = procTree[pid];
					if (proc) {
						proc.files.push(pf.substring(1));
					}
				}
			});
			for (const i in procTree) {
...
function handleGitRequest(request, response, git) {
	const segs = git.split("/");
	let repoIdx = segs.length-1;
	if (segs[repoIdx] === '.git') {
		repoIdx--;
	}
	const repoName = segs[repoIdx];
...
function handleRequest(request, response){
	...
	let dir = decodeURIComponent(url.pathname).replace(/\/$/, '');
	if (dir === '') {
		dir = '/';
	}

	const depth = url.query.depth || 1;
