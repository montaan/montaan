...
function w32(buf, v, off) {
	buf[off] = (v >> 24) & 255;
	buf[off + 1] = (v >> 16) & 255;
	buf[off + 2] = (v >> 8) & 255;
	buf[off + 3] = v & 255;
	return off + 4;
} // Write a 32-bit big-endian int to buffer at offset and return the offset after it.
function w16(buf, v, off) {
	buf[off] = (v >> 8) & 255;
	buf[off + 1] = v & 255;
	return off + 2;
} // Write a 16-bit big-endian int to buffer at offset and return the offset after it.

class Client {
	// The Client class wraps a connection socket to a PostgreSQL database and speaks the protocol.
	constructor(config) {
		// Client constructor takes a config object that contains parameters to pass to the server.
		if (!(config.user && config.database))
			throw Error("You need to provide both 'user' and 'database' in config"); // PostgreSQL protocol requires at least a user and database.
	...
	connect(address, host, ssl) {
		// Connects to either a UNIX socket or a TCP socket, with optional SSL encryption. See Node.js net.createConnection.
		...
		// Ends the connection by terminating the PostgreSQL protocol and disconnecting.
		this.zeroParamCmd(88); // X -- Terminate -- Send terminate message to the server.
		return this._connection.destroy(); // Disconnect socket.
	}
	_connect() {
		// Internal connect helper, creates a connection based on connection parameters.
		...
		this._connection.once('connect', () => {
			// Upgrade to SSL on connection if wanted.
			if (this.ssl) {
				// We have ssl config, try to establish an SSL connection.
				this._connection.once('data', (buffer) => {
					// Deal with the server's response to our SSL request.
					...
					this._connection = tls.connect(
						{ socket: this._connection, ...this.ssl },
						this.onConnect.bind(this)
					); // OK, upgrade to an SSL socket and make it handle the protocol.
	...
	onError(err) {
		// Deal with errors in the connection.
		if (err.message.startsWith('connect EAGAIN ')) this._connect();
		// If you try to open too many UNIX sockets too quickly, the Linux syscall sends an EAGAIN interrupt asking you to try again.
		else this._queryHandlers.splice(0).forEach((s) => s.reject(err)); // Clear out the query result handlers and reject them all.
		this._connect(); // Try to reconnect after a second.
	}
	onConnect() {
		// Initiate connection with PostgreSQL server by sending the startup packet.
		if (this.config.cancel) {
			// Unless this connection is for canceling a long-running request.
			this._connection.write(
				Buffer.concat([Buffer.from([0, 0, 0, 16, 4, 210, 22, 46]), this.config.cancel])
			); // Send a CancelRequest with our backendKey cancel token.
		...
		for (const n in this.config)
			if (n !== 'password') chunks.push(Buffer.from(`${n}\0${this.config[n]}\0`)); // Send config to the server, except for the password. It is handled in the Authentication flow.
	...
	onData(buf) {
		// Handle data received from PostgreSQL server.
		for (let i = 0; i < buf.byteLength; ) {
			...
			if (this._packet.cmd === 0) {
				...
				switch (this._packet.cmd) {
					...
					case 84:
					case 49:
					case 50: // DataRow / CopyData / RowDescription / CommandComplete
				...
				if (this._packet.index === 4) {
					...
					if (this.streamStart === -1) {
						// Deal with packet directly if we're not streaming.
						if (this._packet.length === 4) {
							// If the packet has no payload, use a cached buffer.
							this._packet.buf = this._zeroParamMsgBufs[this._packet.cmd];
							if (!this._packet.buf)
								this._zeroParamMsgBufs[
									this._packet.cmd
								] = this._packet.buf = Buffer.from([this._packet.cmd, 0, 0, 0, 4]);
							this.processPacket(
								this._packet.buf,
								this._packet.cmd,
								this._packet.length,
								5,
								this._queryHandlers[0]
							); // Pass packet to protocol parser.
							this._packet.cmd = this._packet.index = this._packet.length = 0;
							this._packet.buf = null; // Reset the packet.
							continue;
						} else if (this._packet.cmd === 90) {
							this._packet.buf = this._readyForQueryBuf;
						} else {
							// Otherwise allocate a new buffer and stuff the packet there.
							...
							this._packet.buf[1] = this._packet.head[0];
							this._packet.buf[2] = this._packet.head[1];
							this._packet.buf[3] = this._packet.head[2];
							this._packet.buf[4] = this._packet.head[3];
						}
						// } else {
			...
			if (this._packet.index >= 4) {
				...
				if (i < buf.byteLength) {
					if (this.streamStart === -1) {
						// Not streaming, copy packet contents here.
						if (this._packet.cmd === 90) {
							this._packet.index++;
							this._packet.buf[5] = buf[i++];
						} else {
							const copiedBytes = buf.copy(
								this._packet.buf,
				...
				if (this._packet.index === this._packet.length) {
		...
		if (this.streamStart !== -1) {
			// Are we writing to a receiving stream?
			// console.log('write to stream', this.streamStart, buf.byteLength);
			if (this.streamStart === 0) this._queryHandlers[0].stream.write(buf);
			// Pass the whole buffer if we're starting from 0
			else this._queryHandlers[0].stream.write(buf.slice(this.streamStart)); // If starting from mid-buffer, slice instead.
			this.streamStart = 0;
		}
	}
	processPacket(buf, cmd, length, off, queryHandler) {
		switch (
			cmd // Process a protocol packet and write it to the output stream if needed.
		) {
			...
			case 115:
			case 71:
			case 87: // PortalSuspended / CopyInResponse / CopyBothResponse -- Two-part query flow messages. Resolve promises and get written to the output stream.
			...
			case 73:
			case 72:
			case 99: // EmptyQueryResponse / CopyOutResponse / CopyDone -- These are written to the output stream for further processing.
				queryHandler.stream.write(buf);
				break;
			case 110:
			case 116:
			case 49:
			case 51: // NoData / ParameterDescription / {Parse,Bind,Close}Complete -- These are ignored.
				break;
			case 69: // E -- Error -- There was an error in handling the query, reject the current queryHandler promise.
				...
				queryHandler.reject(
					Error(
						`PostgreSQL Error: ${buf[off]} ${buf
							.toString('utf8', off + 1, off + length - 4)
							.replace(/\0/g, ' ')}`
					)
				); // Reject the current query with the error message from the server.
				break;
			...
			case 82: // R -- Authentication -- Server either asks us for a password or tells that everything's A-OK, come on in friend.
				const authResult = r32(buf, off);
				off += 4; // Read the auth message code and deal with it.
				if (authResult === 0) this.authenticationOk = true;
				// Authentication successful.
				else if (authResult === 3) {
					// 3 -- AuthenticationCleartextPassword -- Server asks us to send a password in clear text.
				...
				} else if (authResult === 5) {
					// 5 -- AuthenticationMD5Password -- Server asks us for a MD5-hashed password.
					if (this.config.password === undefined) throw Error('No password supplied'); // For which we need a password.
					const upHash = crypto
						.createHash('md5')
						.update(this.config.password)
						.update(this.config.user)
						.digest('hex'); // Hash the password and username.
					const salted = crypto
						.createHash('md5')
						.update(upHash)
						.update(buf.slice(off, off + 4))
						.digest('hex'); // Then hash that hash with the salt from the server.
					...
					this.end();
					throw Error(`Authentication method ${authResult} not supported`);
				} // Other auth mechanisms are not implemented.
				break;
			...
			case 118: // NegotiateProtocolVersion -- Server requests us to adapt to a different minor protocol version. We won't.
				this.end();
				throw Error('NegotiateProtocolVersion not implemented'); // Terminate connection and bail out.
			case 86: // FunctionCallResponse -- Legacy, not supported, use 'SELECT my_func()' instead. How did you manage to do a FunctionCall in the first place?
				this.end();
				throw Error('FunctionCallResponse not implemented'); // Terminate connection and bail out.
			default:
				// Unknown command. Our code is broken. Or the server is broken. Anyway, it's not ACID, eject eject eject.
				console.error(
					cmd,
					String.fromCharCode(cmd),
					length,
					buf.toString('utf8', off, off + length - 4)
				); // Log the command to stderr.
				this.end();
				throw Error('Unknown message. Protocol state unknown, exiting.'); // Terminate connection and bail out.
		}
	}
	promise(stream, parsed) {
		return new Promise((resolve, reject) =>
			this._queryHandlers.push({ stream, parsed, resolve, reject })
		);
	} // Create a new query handler for current query.
	parseAndDescribe(statementName, statement, types = []) {
		// Parses a query string and requests a query results description. Rolled into a single write for performance.
	...
	_writeExecuteBuf(msg, portalNameBuf, maxRows, off) {
		// Writes an Execute message to the msg buffer at given offset off.
	...
	_bindBuf(portalNameBuf, statementNameBuf, values, resultFormat, extraBytes) {
		...
		if (values.rowBuffer) {
			// Values is a row from a previous query, we can copy it over directly.
			...
			// Prepare the parameter values for the bind message and calculate message length.
			values = values.slice(); // Copy values and convert them to Buffers.
			for (let i = 0; i < values.length; i++) {
				// Go through the copy of values, converting values to buffers and determining value formats.
				valueFormats[i] = values[i] instanceof Buffer ? 1 : 0; // Buffer parameters are passed as binary to PostgreSQL.
				if (values[i] != null)
					(values[i] = valueFormats[i] ? values[i] : Buffer.from(values[i])),
						(bytes += values[i].byteLength); // Convert non-null/undefined values to buffers and add their byteLength to total number of bytes.
			}
		...
		let off = 5;
		msg[0] = 66; // B -- Bind -- Write Bind command at the start of the message. Skip off to after the message header.
		...
		if (values.rowBuffer) off += values.rowBuffer.copy(msg, off, 5);
		// If we're dealing with a row from a RowParser, copy its body to the message.
		else {
			// Otherwise go through the values and write them out.
			off = w16(msg, values.length, off); // The values array starts with a i16 length.
			for (let i = 0; i < values.length; i++) {
				// And is followed by a i32 length and a payload for each element.
				if (values[i] == null) off = w32(msg, -1, off);
				// Null/undefined values have a length of -1 and no payload.
	...
	bind(portalNameBuf, statementNameBuf, values = [], resultFormat = Client.STRING) {
		// Bind the parameters to a query. Return the results in the given format.
		const { msg, off } = this._bindBuf(
			portalNameBuf,
			statementNameBuf,
			values,
			resultFormat,
			0
		); // Create the Bind message buffer.
	...
	bindExecuteSync(
		...
		maxRows = 0,
		values = [],
		resultFormat = Client.STRING
	) {
		// Bind the parameters to a query, execute it and sync, using a single write. Return the results in the given format.
		let { msg, off } = this._bindBuf(
			portalNameBuf,
			statementNameBuf,
			values,
			resultFormat,
			14 + portalNameBuf.byteLength
		); // Create the Bind message buffer with enough space for the execute and sync at the end.
		off = this._writeExecuteBuf(msg, portalNameBuf, maxRows, off); // Write the execute message to the message buffer after the bind.
		msg[off] = 83;
		msg[++off] = 0;
		msg[++off] = 0;
		msg[++off] = 0;
		msg[++off] = 4; // S -- Sync -- Write a sync message to the end of the message buffer.
		this._connection.write(msg); // Send the bind, execute and sync messages to the server.
	}
	executeFlush(portalNameBuf, maxRows) {
		// Executes a portal and sends a flush to the server so that it doesn't buffer the results. (Important for partial result queries.)
		...
		msg[++off] = 0;
		msg[++off] = 0;
		msg[++off] = 4; // H -- Flush -- Write the flush to the end of the message.
		this._connection.write(msg); // Send the execute and flush to the server.
	}
	close(type, name) {
		return this.bufferCmd(67, Buffer.from(type + name + '\0'));
	} // C -- Close -- Closes a named Portal or a prepared Statement.
	authResponse(buffer) {
		return this.bufferCmd(112, buffer);
	} // p -- PasswordMessage/GSSResponse/SASLInitialResponse/SASLResponse -- Send an authentication response to the server.
	bufferCmd(cmd, buffer) {
		// Helper for commands where the body is a Buffer.
		const msg = Buffer.allocUnsafe(5 + buffer.byteLength); // Allocate message buffer. 5B message header followed by the buffer.
		msg[0] = cmd;
		w32(msg, 4 + buffer.byteLength, 1); // Write the message command and length to the header.
		buffer.copy(msg, 5); // Write the buffer contents after the header.
		this._connection.write(msg); // Send the command to the server.
	}
	zeroParamCmd(cmd) {
		// Helper for commands with no payload.
	...
	parseStatement(statement, cacheStatement) {
		// Parses a query statement and caches it as a prepared statement if cacheStatement is true.
		let parsed = this._parsedStatements[statement]; // Have we cached this already?
		if (!parsed) {
			// If the statement is not cached, let's prepare it.
			parsed = {
				name: cacheStatement ? (this._parsedStatementCount++).toString() : '',
				rowParser: null,
			}; // Create statement descriptor. If cacheStatement is true, create a uniquely named prepared statement.
	...
	startQuery(statement, values = [], format = Client.STRING, cacheStatement = true) {
		// Starts a partial results query. Call getResults to request the next batch of results.
		if (this.inQuery)
			throw Error('Already in query. Please sync() before starting a new query.');
	...
	getResults(maxCount = 0, stream = new RowReader()) {
		// Get the next maxCount results from a partial query. Use 0 as maxCount to request all remaining rows.
	...
	query(
		...
		cacheStatement = true,
		stream = new RowReader()
	) {
		// Send a query to the server and receive the results to the given stream in the given format.
	...
	copyData(buffer) {
		return this.bufferCmd(100, buffer);
	} // d -- CopyData -- Send a copy data buffer to the server.
	copyFail(buffer) {
		this.bufferCmd(102, buffer);
		return this.promise(null, null);
	} // f -- CopyFail -- Tell the server that the copy operation has failed for some reason.
	copyDone(stream = new CopyReader()) {
		// Tell the server that we've finished copying data to the server.
		const msg = Buffer.allocUnsafe(10); // Allocate a message buffer for the CopyDone and Sync messages.
		msg[0] = 99;
		msg[1] = 0;
		msg[2] = 0;
		msg[3] = 0;
		msg[4] = 4; // c -- CopyDone -- Write the CopyDone message at the start of the buffer.
		msg[5] = 83;
		msg[6] = 0;
		msg[7] = 0;
		msg[8] = 0;
		msg[9] = 4; // S -- Sync -- Write a Sync message after CopyDone to commit the transaction.
	...
	sync(stream = new RowReader()) {
		this.zeroParamCmd(83);
		return this.promise(stream, null);
	} // Sends a Sync command to the server and returns a promise of ready for query.
	cancel() {
		return new Client({ ...this.config, cancel: this.backendKey }).connect(
			this.address,
			this.host,
			this.ssl
		);
	} // Cancels the currently running request and returns a promise of the cancellation.
}
...
class RowReader {
	...
	constructor() {
		(this.rows = []),
			(this.cmd = this.oid = this.binary = this.rowParser = undefined),
			(this.rowCount = 0);
	...
	write(buf, off = 0) {
		switch (
			...
			case 68:
				return this.rows.push(new this.rowParser[this.binary](buf)); // D -- DataRow -- Parse the row and store it to rows.
			case 100:
				return this.rows.push(buf.slice(off + 4, off + r32(buf, off))); // CopyData -- Slice the copy message payload and store it to rows.
			case 67: // C -- CommandComplete -- Parse the command complete message and set our completion state.
				...
				return (
					(this.cmd = cmd), (this.oid = oid), (this.rowCount = parseInt(rowCount || 0))
				); // Store the parsed data and exit.
			case 73:
				return (this.cmd = 'EMPTY'); // I -- EmptyQueryResult -- Command completion message for an empty query string.
			case 115:
				return (this.cmd = 'SUSPENDED'); // s -- PortalSuspended -- Completion message for a result batch from a partial results query.
			case 99:
				return (this.cmd = 'COPY'); // CopyDone -- Completion message for a copy command.
			case 71:
			case 87:
			case 72: // Copy{In,Both,Out}Response -- Copy response describes the number of columns of the following CopyData messages and their formats (0 = string, 1 = binary.)
				this.format = buf[off + 4];
				off += 5; // Grab the global format code (0 = string, 1 = binary) and skip the message header.
				this.columnCount = r16(buf, off);
				off += 2; // Read the number of columns in the copy message.
				this.columnFormats = []; // Store the column formats in an array.
				for (let i = 0; i < this.columnCount; i++)
					(this.columnFormats[i] = r16(buf, off)), (off += 2); // Read the column formats (0 = string, 1 = binary.)
		}
	}
}
class RowParser {
	...
	constructor(buf, off = 0) {
		...
		const columnCount = r16(buf, off + 5);
		off += 7; // Read in the array of column descriptions.
		for (let i = 0; i < columnCount; i++) {
			...
			off = nameEnd + 1; // Then extract the string without the null byte.
			const tableOid = r32(buf, off);
			off += 4; // If the column is from a table, here's the table's OID. Otherwise zero.
			const tableColumnIndex = r16(buf, off);
			off += 2; // Along with the index of the column in the table. If the column is not from a table, this is zero.
			const typeOid = r32(buf, off);
			off += 4; // Next comes the PostgreSQL data type OID of the column. SELECT typname, oid, typarray FROM pg_type WHERE typarray != 0;
			const typeLen = r16(buf, off);
			off += 2; // The byteLength of a fixed size type. -1 means variable length type.
			const typeModifier = r32(buf, off);
			off += 4; // The type modifier, see pg_attribute.atttypmod.
			const binary = r16(buf, off);
			off += 2; // Whether the column is sent as binary (0 = string, 1 = binary.)
			const column = {
				name,
				tableOid,
				tableColumnIndex,
		...
		for (let i = 0; i < this.columns.length; i++) {
			...
			const getter = {
				get: function() {
					...
					let off = 7,
						buf = this.rowBuffer; // Set up walk state.
					if (this.columnOffsets === undefined) {
						...
						for (let i = 0; i < this.columnCount; i++) {
							...
							const columnLength = r32(buf, off);
							off += 4; // Read in the column length.
		...
		this[Client.BINARY] = function(buf) {
			this.rowBuffer = buf;
		...
		(this[Client.BINARY].prototype.dataFormat = Client.BINARY), // The binary rows have a binary data format.
			(this[Client.BINARY].prototype.parseColumn = function(start, end) {
				return this.rowBuffer.slice(start, end);
			}), // Parsing a binary column is just a slice.
			(this[Client.STRING] = function(buf) {
				this.rowBuffer = buf;
		...
		this[Client.STRING].prototype.parseColumn = function(start, end) {
			return this.rowBuffer.toString('utf8', start, end);
...
RowParser.parserPrototype = {
	toArray: function() {
		...
		const off = 7,
			buf = this.rowBuffer,
			dst = new Array(this.columnCount); // Create parsing state and result array.
		for (let i = 0; i < this.columnCount; i++) {
			// Go through the columns.
			const columnLength = r32(buf, off);
			off += 4; // Each column has an i32 length, followed by the column data.
			if (columnLength >= 0) {
				dst[i] = this.parseColumn(off, off + columnLength);
				off += columnLength;
	...
	toObject: function() {
		...
		const off = 7,
			buf = this.rowBuffer,
			dst = {}; // Set up parsing state and result object. Off 7 skips over the header and column count (which we know already.)
		for (let i = 0; i < this.columnCount; i++) {
			// Go through the columns.
			const columnLength = r32(buf, off);
			off += 4; // Read the length of the column.
			if (columnLength >= 0) {
				dst[this.rowColumns[i].name] = this.parseColumn(off, off + columnLength);
				off += columnLength;
